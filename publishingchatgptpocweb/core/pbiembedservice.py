...
from core.aadservice import AadService
from models.pbi.reportconfig import ReportConfig
from models.pbi.embedtoken import EmbedToken
from models.pbi.embedconfig import EmbedConfig
from models.pbi.embedtokenrequestbody import EmbedTokenRequestBody
...
class PbiEmbedService:
    ...
    def update_report_parameter(self, workspace_id, dataset_id, parameter_name, parameter_value):
        ...
        Args:
            ...
            parameter_value (str): New parameter value
        '''

        update_params_url = f'https://api.powerbi.com/v1.0/myorg/groups/{workspace_id}/datasets/{dataset_id}/UpdateParameters'
        body = {
            "updateDetails": [
                {
                    "name": parameter_name,
                    "newValue": parameter_value
                }
            ]
        }

        api_response = requests.post(update_params_url, headers=self.get_request_header(), json=body)
        
        if api_response.status_code != 200:
            abort(api_response.status_code, description=f'Error while updating report parameter\n{api_response.reason}:\t{api_response.text}\nRequestId:\t{api_response.headers.get("RequestId")}')
    ...
    def refresh_dataset(self, workspace_id, dataset_id):
        ...
        refresh_dataset_url = f'https://api.powerbi.com/v1.0/myorg/groups/{workspace_id}/datasets/{dataset_id}/refreshes'

        api_response = requests.post(refresh_dataset_url, headers=self.get_request_header())
        
        if api_response.status_code != 202:
            abort(api_response.status_code, description=f'Error while refreshing dataset\n{api_response.reason}:\t{api_response.text}\nRequestId:\t{api_response.headers.get("RequestId")}')
